...
def tract_and_rearrange(string): #FUNCTION WAS NOT DEFINED and spellimg mistakes
    ...
    str_3 = "".join("".join(reversed(list(string[14:20]))).split(string[17])) #JOIN("" NOT NEEDED and it shoudld be str_3
    str_4 = "".join(string[21:29].split(string[23:28]))

    return str_1 + " " + str_2 + " " + str_3 + " " +str_4 #the plus sign was missing

def ultra_extrct_and_rearrange(string): #missing colon after defining the function and spelling mistakes
